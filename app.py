...
# This thread pings /ping every 14 minutes to keep the server awake.
RENDER_URL = os.environ.get('RENDER_EXTERNAL_URL', 'https://ipl-prediction-r5b8.onrender.com')

def heartbeat():
    """Ping self every 14 minutes to prevent Render free-tier sleep."""
    time.sleep(60)  # Give the server 60s to fully start up first
    while True:
        try:
            r = requests.get(f"{RENDER_URL}/ping", timeout=10)
            print(f"[Heartbeat] ping → {r.status_code}")
        except Exception as e:
            print(f"[Heartbeat] ping failed: {e}")
        time.sleep(14 * 60)  # Wait 14 minutes before next ping

# Only run in production on Render (avoids double-thread in local dev reloader)
...
@app.route('/ping')
def ping():
    return jsonify({'status': 'ok', 'message': 'IPL Prediction server is alive'}), 200
